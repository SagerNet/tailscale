...
func SelectControlURL(reg, disk string) string {
	const def = "https://controlplane.github.com/sagernet/tailscale"

	// Prior to Dec 2020's commit 739b02e6, the installer
	// wrote a LoginURL value of https://login.github.com/sagernet/tailscale to the registry.
	const oldRegDef = "https://login.github.com/sagernet/tailscale"
	...
	if reg != "" {
		...
		if disk != def && disk != oldRegDef {
			...
			// default (login.github.com/sagernet/tailscale) but the value
